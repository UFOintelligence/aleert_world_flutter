...
abstract class AlertRemoteDataSource {
  Future<List<AlertEntity>> obtenerAlertas(); // <- CORREGIDO
}
class AlertRemoteDataSourceImpl implements AlertRemoteDataSource {
  ...
  Future<List<AlertEntity>> obtenerAlertas() async {
    final response = await client.get(Uri.parse('$baseUrl/auth/alertas'));
    ...
    if (response.statusCode == 200) {
      ...
      return models.map((m) => m.toEntity()).toList(); // ✅ CORRECTO
    } else {
